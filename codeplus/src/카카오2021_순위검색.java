...
public class 카카오2021_순위검색 {
	...
	static Query getQueryByName(String queryString) {
		for (Query query: querySet) {
			if (query.isMatchedString(queryString)) {
				return query;
			}
		}
		return null;
	...
	static void dfs(String queryString, int depth, String[] info) {
		if (depth == 4) {
			if (getQueryByName(queryString) == null) {
				Query query = new Query(queryString);
				query.addScore(Integer.parseInt(info[4]));
				querySet.add(query);
			} else {
				Query query = getQueryByName(queryString);
				assert query != null;
				query.addScore(Integer.parseInt(info[4]));
			}
			return;
	...
	static int binarySearch(String queryString, int score) {
		...
		List<Integer> scoreList = Objects.requireNonNull(getQueryByName(queryString)).scoreList;
		int start = 0;
